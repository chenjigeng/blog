...
最近看了[Dan Abramov](https://mobile.twitter.com/dan_abramov)的一些[博客](<https://overreacted.io/>)，学到了一些React的一些有趣的知识。决定结合自己的理解总结下。接下来直接进入正文。
...
不知道你有没有察觉，我们写React类组件的时候，我们都需要通过extends React.Component的方式来写。那么，我们是否可以通过以下方式来判断呢？
...
- 对于一些项目(虽然很少)可能存在着多个React副本，并且我们目前要检查的组件它继承的React.Component是来自于另一个React副本的，这就会出现问题。

### 通过为React.Component增加一个特别的标记

写过React类组件的人都知道，我们每一个类组件都是要继承于React.Component的。因此，如果我们在React.Component增加一个标记`isReactComponent`，这样通过继承的方式，我们就可以根据这个标记来判断是不是类组件了。
...
事实上，[React](<https://github.com/facebook/react/blob/769b1f270e1251d9dbdce0fcbd9e92e502d059b8/packages/react-reconciler/src/ReactFiber.js#L297-L300>)目前就是通过这种方式来进行检查的。如果你没有`extends React.Component`，React不会在原型上找到`isReactComponent`，因此不会把组件当做类来处理。
...
之后，这个函数执行结果会返回一个对象，这个对象我们称为React Element，想了解更多与React Component,Elements和Inastances的可以[点击这里](<https://reactjs.org/blog/2015/12/18/react-components-elements-and-instances.html>)。
...
对于React开发者来说，上面这些属性都是比较常见的。可是为什么混进了一个`$$typeof`？？它是干嘛的呢？它的值为什么是一个`Symbol`呢？
